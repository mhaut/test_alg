...
def main():
    ...
    parser.add_argument('--dataset', type=str, required=True, \
            choices=["IP", "UP", "SV", "KSC", "UH", "DIP", "DUP", "DIPr", "DUPr"], \
            help='dataset (options: IP, UP, SV, KSC, UH, DIP, DUP, DIPr, DUPr)')
    parser.add_argument('--repeat', default=1, type=int, help='Number of runs')
    parser.add_argument('--idtest', default=0, type=int, help='idrun')
    ...
    for pos in range(args.repeat):
        ...
        results = mymetrics.reports(clf.predict(x_test), y_test)[2]
        print("SVM", args.dataset, args.tr_percent, args.idtest, tiempo, list(results))
